...
#' @param fixed_R an evolutionary rate matrix to be used for the computation.
...
#' @param post_score_samples The number of posterior samples used to compute the posterior of scores.
...
#'
#' @export
MCMCphylProbPCA <- function(phy, x, ret_dim = 2, scale = TRUE, fixed_R = NULL, gamma_shape = 1, gamma_rate = 2, gen = 10^6, step_scale = 0.08, burn = 0.2, post_score_samples = 100){
  ...
  ## Check if R should be estimated or used as given.
  if( is.null(fixed_R) ){
  ...
  ## Add the varnames for the posterior:
  mcmc_varnames <- c(paste("L", 1:ret_dim, sep = ""), "sigma")

  ## ###########################################################################
  ## Go over the posterior samples and compute the scores:
  message("Computing scores for posterior samples...")
  mcmc_score_list <- getPostProjection(post = post, R_decom = R_decom, q = q
                                       , x = x, invC = invC, R = R, W = W, a = a
                                       , N = N, d = d, post_n = post_score_samples)
  message("Done!")

  ## Need to create class for the output object!
  ## Need to first decide on the function to plot the posterior distribution. Then we can tune the output list and create a new output class.
  out <- list(mean_scores = mean_scores, post_scores = mcmc_score_list
              , percent_var_pcs = mcmc_pc_var
              , var_loadings = mcmc_loadings, mcmc = mcmc1
              , stats = mcmc_proj, mcmc_varnames = mcmc_varnames)
  return( out )
}
